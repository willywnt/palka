...
/** What the importer will do with one row. */
export type ProductImportStatus = 'create' | 'update' | 'skip' | 'error';

/** The editable/validatable columns an import error can attach to. */
...
export type ProductImportRowResult = {
  /** Stable 1-based source line (header = line 1); survives edits, used as a key. */
  line: number;
  status: ProductImportStatus;
  /** The SKU after planning — auto-generated when the input was blank; null if N/A. */
  resolvedSku: string | null;
  /** True when `resolvedSku` was generated by the system (input SKU was blank). */
  skuGenerated: boolean;
  productName: string;
  variantName: string;
  /** Per-column validation errors (shown inline on each cell). */
  fieldErrors: Partial<Record<ProductImportField, string>>;
  /** Residual row-level note (skip reason / rare cross-column issue); null otherwise. */
  message: string | null;
};
...
/**
 * Existing-data maps for the import preview — the wizard fetches these once so it
 * can plan (create-vs-update) and re-plan edits instantly on the client.
 */
export type ProductImportContextData = {
  variantsBySku: Record<string, { variantId: string; productId: string }>;
